...
//    self.navigationItem.rightBarButtonItem = cancelItem;
    UIBarButtonItem *selectItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Select all", nil) style:UIBarButtonItemStylePlain target:self action:@selector(selectAllButtonClick:)];
    [TZCommonTools configBarButtonItem:selectItem tzImagePickerVc:tzImagePickerVc];
    self.navigationItem.rightBarButtonItems = @[cancelItem, selectItem];
...
- (void)configCollectionView {
    if (!_collectionView) {
        ...
        if (tzImagePickerVc.collectionViewBgColor) {
            _collectionView.backgroundColor = tzImagePickerVc.collectionViewBgColor;
        } else {
            if (@available(iOS 13.0, *)) {
...
- (void)configBottomToolBar {
    ...
    if (tzImagePickerVc.toolBarBgColor) {
        _bottomToolBar.backgroundColor = tzImagePickerVc.toolBarBgColor;
    } else {
    ...
    _divideLine.backgroundColor = [UIColor blackColor];
...
- (void)selectAllButtonClick:(UIBarButtonItem *)sender {
    
    BOOL isSelectAll = [sender.title isEqualToString:NSLocalizedString(@"Select all", nil)];
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    for (TZAssetModel *model in _models) {
        ...
        if (!isSelectAll) {
            if ([tzImagePickerVc.selectedModels containsObject:model]) [tzImagePickerVc removeSelectedModel:model];
        } else {
            if (![tzImagePickerVc.selectedModels containsObject:model]) [tzImagePickerVc addSelectedModel:model];;
        }
        ...
        [self setAsset:model.asset isSelect:isSelectAll];
    }
    
    sender.title = isSelectAll ? NSLocalizedString(@"Unselect all", nil) : NSLocalizedString(@"Select all", nil);
    [self.collectionView reloadData];
...
- (void)refreshBottomToolBarStatus {
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    if (tzImagePickerVc.selectedModels.count == _models.count) {
        UIBarButtonItem *selectItem = self.navigationItem.rightBarButtonItems.lastObject;
        [selectItem setTitle:NSLocalizedString(@"Unselect all", nil)];
    }
